...
export async function createServer({ dbPath, uploadsPath, jwtSecret }) {
  ...
  app.use('/api/closures', authenticate, closuresRouter);
  app.use('/api', authenticate, transactionsRouter);

  app.use((err, _req, res, _next) => {
    console.error(err);
    // err.status permite a una ruta marcar su propio código (ej. 409 cuando
    // decrementInventory rechaza una venta por stock insuficiente) sin que
    // este middleware genérico lo aplaste con un 500.

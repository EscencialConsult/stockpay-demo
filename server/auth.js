  ...
  return jwt.sign(
    {
      ...
      role: user.role,
      // Los flags perm_* vienen calculados desde el rol en `mapUser` — se
      // repiten acá tal cual para no tocar las rutas que ya los leen de
      // `req.user[perm]`.
...
export function requirePerm(perm) {
  return (req, res, next) => {
    ...
    if (req.user.role === 'administrador' || req.user[perm]) {
      return next();
    }
...
export function requireAnyPerm(...perms) {
  return (req, res, next) => {
    ...
    if (req.user.role === 'administrador' || perms.some((perm) => req.user[perm])) {
      return next();
    }
    return res.status(403).json({ error: 'Permiso denegado' });
  };
